...
public class ArrayEx6 {
	...
	public int search(int row, int num) {
		return search(row, num, 0);
	}
	public int search(int row, int num, int fromIndex) {
		...
		// 못찾으면 -1
		for(int i=fromIndex; i<m[row].length; i++) {
	...
	public void delete(int row, int num, String all) {
		// char 추가로 받아서 allDelete도 되도록 수정가능
		// 숫자존재유무 검색
		int index = search(row, num);
		// 없으면 끝
		if(index==-1) return ;
		// 해당숫자 뒤에수들 땡기기

...
-- ex10)hello bit !!!
select insert('helloabc!!!', 6, 3, ' bit ')
from dual;

-- ex11) 바꾸기(replace)
select replace('helloabc!!!', 'abc', ' bit ')
from dual;

-- ex12) 문자열의 위치(instr)
select instr('hello bit !!!', 'bit')
from dual;

-- ex13)위치부터 갯수만큼리턴(mid, substring)
select mid('hello bit !!!', 7, 5), substring('hello bit !!!', 7, 5)
from dual;
...
select lower('hELlo bit !!!'), lcase('hELlo bit !!!')
from dual;

-- ex16)대문자로 바꾸기(upper, ucase)
select upper('hELlo bit !!!'), ucase('hELlo bit !!!')
from dual;

-- ex17)왼쪽에서 개수만큼추출(left),  오른쪽에서 개수만큼추출(right)
select left('hello bit !!!', 5), right('hello bit !!!', 6)
from dual;

-- ex18)
# 2023-04-06 13:30:08,    2023-04-06 13:30:08,    2023-04-06 13:30:08
select now(), sysdate(), current_timestamp()
from dual;

-- ex19)
# 2023-04-06,   2023-04-06,   13:29:51,   13:29:51
...
select count(employee_id), sum(salary), avg(salary), min(salary), max(salary)
from employees;
-- -----------------------------------------------------------------
-- [11]문제풀기
...
insert into student (name, kor, eng, mat) values
                   ('둘리', 90, 80,95),
         ('또치', 75, 90,65),
         ('고길동', 100,95,90),
         ('마이콜', 60,60,60),
...
select idx '학번', name '이름', kor '국어', eng '영어', mat '수학', (kor+eng+mat) '총점', truncate((kor+eng+mat)/3,2) '평균'
from student;
